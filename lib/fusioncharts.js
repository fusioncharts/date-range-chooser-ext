...
FusionCharts.prototype.getFormattedDate = function (timestamp) {
  return new Date(timestamp).toLocaleDateString();
};

FusionCharts.prototype.getTimestamp = function (date) {
  ...
  obj.init(function () {
    ...
    for (var i = 0; i < arguments.length - 1; i++) {
      if (arguments[i] === 'X-Axis') {
        ...
        argsToApply.push(paper);
      } else if (arguments[i] === 'chart') {
        argsToApply.push(new FusionCharts());
      }
    }
